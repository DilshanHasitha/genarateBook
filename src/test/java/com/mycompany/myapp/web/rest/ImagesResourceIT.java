...
class ImagesResourceIT {
    ...
    @Test
    @Transactional
    void getAllImagesByStoreTypeIsEqualToSomething() throws Exception {
        ImageStoreType storeType;
        if (TestUtil.findAll(em, ImageStoreType.class).isEmpty()) {
            imagesRepository.saveAndFlush(images);
            storeType = ImageStoreTypeResourceIT.createEntity(em);
        } else {
            storeType = TestUtil.findAll(em, ImageStoreType.class).get(0);
        }
        em.persist(storeType);
        em.flush();
        images.setStoreType(storeType);
        imagesRepository.saveAndFlush(images);
        Long storeTypeId = storeType.getId();

        // Get all the imagesList where storeType equals to storeTypeId
        defaultImagesShouldBeFound("storeTypeId.equals=" + storeTypeId);

        // Get all the imagesList where storeType equals to (storeTypeId + 1)
        defaultImagesShouldNotBeFound("storeTypeId.equals=" + (storeTypeId + 1));
    }
