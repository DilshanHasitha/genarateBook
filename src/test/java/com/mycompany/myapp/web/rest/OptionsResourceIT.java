...
class OptionsResourceIT {
    ...
    @Mock
    private OptionsRepository optionsRepositoryMock;
    ...
    void getAllOptionsWithEagerRelationshipsIsEnabled() throws Exception {
        when(optionsServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restOptionsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(optionsServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void getAllOptionsWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        restOptionsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(optionsRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    @Test
    @Transactional
    void getAllOptionsByStyleIsEqualToSomething() throws Exception {
        Styles style;
        if (TestUtil.findAll(em, Styles.class).isEmpty()) {
            optionsRepository.saveAndFlush(options);
            style = StylesResourceIT.createEntity(em);
        } else {
            style = TestUtil.findAll(em, Styles.class).get(0);
        }
        em.persist(style);
        em.flush();
        ...
        // Get all the optionsList where style equals to styleId
        defaultOptionsShouldBeFound("styleId.equals=" + styleId);

        // Get all the optionsList where style equals to (styleId + 1)
        defaultOptionsShouldNotBeFound("styleId.equals=" + (styleId + 1));
    }

    @Test
    @Transactional
    void getAllOptionsByAvatarAttributesIsEqualToSomething() throws Exception {
        ...
        em.flush();
        options.addAvatarAttributes(avatarAttributes);
        optionsRepository.saveAndFlush(options);
        Long avatarAttributesId = avatarAttributes.getId();

        // Get all the optionsList where avatarAttributes equals to avatarAttributesId
        defaultOptionsShouldBeFound("avatarAttributesId.equals=" + avatarAttributesId);

        // Get all the optionsList where avatarAttributes equals to (avatarAttributesId + 1)
        defaultOptionsShouldNotBeFound("avatarAttributesId.equals=" + (avatarAttributesId + 1));
    }
